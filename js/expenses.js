...
export async function exportToGoogleSheets(data, sheetName = 'Expenses') {
  const { sheetsConfig } = await import('./firebase-config.js');

  if (!sheetsConfig.webAppUrl || sheetsConfig.webAppUrl === 'YOUR_GOOGLE_APPS_SCRIPT_URL') {
    throw new Error('กรุณาตั้งค่า Google Apps Script URL ใน firebase-config.js');
  }
  ...
  try {
    const response = await fetch(sheetsConfig.webAppUrl, {
      ...
      body: JSON.stringify({ action: 'export', sheetName, data })
    });
